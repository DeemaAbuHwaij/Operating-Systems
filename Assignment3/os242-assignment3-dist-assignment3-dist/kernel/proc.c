...
// Implement a kernel function that, given a virtual address in one process,
// maps the corresponding physical pages to another process
uint64 map_shared_pages(struct proc *src_proc, struct proc *dst_proc, uint64 src_va, uint64 size)
{
    ...
    uint64 dst_va = dst_proc->sz; // Find a Free virtual address in the destination process
    uint64 dst_start = dst_va +(src_va % PGSIZE);
    ...
    dst_proc->sz += (end-start);
    return dst_start;
}

// Implement the corresponding function to unmap the shared memory from
// the destination process
uint64
unmap_shared_pages(struct proc* p, uint64 va, uint64 size) {

  uint64 pages = (PGROUNDUP(size)); 

  if(p->sz < size) {
    return -1;
  }

  uvmunmap(p->pagetable, PGROUNDDOWN(va), (pages/PGSIZE), 0);
  p->sz -= pages;
  return 0;
}
